    ...
    >
      <div className="flex justify-between items-center mx-auto bg-gray-200/20 dark:bg-black/10 text-gray-950 shadow-lg shadow-neutral-600/5 backdrop-blur-lg border border-gray-800/10 md:max-w-5xl rounded-md px-4 py-2">
        <Link to="/" className="p-1">
          <img
            src={logo}
            alt="Sagar Kumar Jha Logo"
            width={112} // Same as w-28
            height={42} // Set based on real aspect ratio
            className="w-28 h-auto invert dark:invert-0"
          />
        </Link>
      ...
      {isMobileMenuOpen && (
        <ul
          id="mobile-menu"
          className="md:hidden absolute w-full h-fit flex flex-col items-center gap-2 py-4 bg-white dark:bg-gray-900 shadow-md border-t border-gray-300 dark:border-gray-700"
        >
          {navItems.map((navItem, i) => (
